function FWRule( $param )
{
	...
	$param.remove('GpoName') >$null
	$gpoptr = New-GPO -Name $GpoName -ErrorAction SilentlyContinue
	...
	$param.remove('Name') >$null
	New-NetFirewallRule -Enabled True -Profile Any -ErrorAction Continue @param >$null
