...
$param = @{
	...
	InlineCode={
		...
		@(
			...
			$path=$_[0]
			$key=$_[1]
			$ret += Get-ItemProperty $path -Name $key -ErrorAction SilentlyContinue | Select @{n='Key';e={$_.PSPath.Replace('Microsoft.PowerShell.Core\Registry::','Malicious hook - ')+"\$key"}},@{n='Value';e={$_."$key"}},@{n='Expected';e={''}},@{n='Compliant';e={$false}}
		}
